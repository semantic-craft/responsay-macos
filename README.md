...
正式发布在维护者本机进行：签名证书与 Sparkle 私钥存放在该机器的登录钥匙串，公证凭证是钥匙串中的 notarytool profile，或本机的一份 App Store Connect 密钥文件。两者都不导出、不进入 CI。操作说明见 [docs/RELEASING.md](docs/RELEASING.md)。
