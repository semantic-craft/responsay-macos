...
/// 小米Mimo 走 `/chat/completions`。开了搜索却用不支持联网的主模型时，提问会被静默退回普通问答、
...
enum VoiceAssistantSearchModelSettings {
    ...
    /// Qwen 走 `/responses` + `LLMSearchControl`;MiMo 走 `/chat/completions`;
    ...
    /// 面向用户的 provider 名(如「阿里云百炼」/「小米Mimo」)。
    ...
    /// 名字用模型品牌(通义千问/MiMo),比 provider 公司名更贴近用户认知。非可联网 → nil。
